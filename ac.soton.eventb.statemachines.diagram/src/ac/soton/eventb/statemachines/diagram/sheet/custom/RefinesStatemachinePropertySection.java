...
/**
 * Refines property section for RefinedStatemachine.
 * 
 * @author vitaly
 *
 */
public class RefinesStatemachinePropertySection extends AbstractEnumerationPropertySection {
	...
	protected EStructuralFeature getFeature() {
		return StatemachinesPackage.Literals.REFINED_STATEMACHINE__REFINES;
	}
	...
	protected String[] getEnumerationFeatureValues() {
		...
		if (abstractMachines.size() > 0) {
			statemachinesToRefine = abstractMachines.get(0).getAllContained(StatemachinesPackage.Literals.ABSTRACT_STATEMACHINE, true);
			String[] values = new String[statemachinesToRefine.size()];
			int i = 0;
			for (EObject sm : statemachinesToRefine) {
				values[i++] = sm == null ? ""
						: sm instanceof Statemachine ? ((Statemachine) sm).getName() 
								: ((RefinedStatemachine) sm).getLabel();
			}
			return values;
		}
		return new String[0];
	...
	protected String getFeatureAsText() {
		AbstractStatemachine statemachine = ((RefinedStatemachine) eObject).getRefines();
		return statemachine == null ? "" 
				: statemachine instanceof Statemachine ? ((Statemachine) statemachine).getName() 
						: ((RefinedStatemachine) statemachine).getLabel();
	}
