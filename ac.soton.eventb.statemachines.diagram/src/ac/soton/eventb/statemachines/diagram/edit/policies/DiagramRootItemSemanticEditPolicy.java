...
import ac.soton.eventb.statemachines.diagram.edit.commands.RefinedStateCreateCommand;
...
public class DiagramRootItemSemanticEditPolicy extends
	...
	protected Command getCreateCommand(CreateElementRequest req) {
		if (StatemachinesElementTypes.Initial_2001 == req.getElementType()) {
			return getGEFWrapper(new InitialCreateCommand(req));
		}
		if (StatemachinesElementTypes.Final_2002 == req.getElementType()) {
			return getGEFWrapper(new FinalCreateCommand(req));
		}
		if (StatemachinesElementTypes.ANY_2003 == req.getElementType()) {
			return getGEFWrapper(new ANYCreateCommand(req));
		}
		if (StatemachinesElementTypes.State_2004 == req.getElementType()) {
			return getGEFWrapper(new StateCreateCommand(req));
		}
		if (StatemachinesElementTypes.RefinedState_2005 == req.getElementType()) {
			return getGEFWrapper(new RefinedStateCreateCommand(req));
		}
