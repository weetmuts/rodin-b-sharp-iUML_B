...
//					RemoveCommand.create(editingDomain, eObject, getFeature(), object));
		...
		table.addMouseListener(new MouseAdapter() {
			...
			public void mouseDoubleClick(MouseEvent e) {
				if (table.getSelection().length > 0) {
					...
					if (selection != null) {
						((ClassdiagramsDiagramEditor) getPart()).getDiagramGraphicalViewer().setSelection(selection);
						((ClassdiagramsDiagramEditor) getPart()).setFocus();
					}
				}
			}
		});
	...
	public void refresh() {
		...
		for (Iterator<? extends EventBElement> i = getOwnedRows().iterator(); i.hasNext();) {
			Object next = i.next();
	 ...
	 */
	protected abstract List<? extends EventBElement> getOwnedRows();
